  ...
  for (const source of VALID_REVIEW_SOURCES) {
    ...
    console.log(
      [
        ...
        `providerRaw=${adapter?.providerRawResultsCount ?? adapter?.rawResultCount ?? 0}`,
        `afterUrl=${adapter?.afterUrlFilterCount ?? adapter?.sourceFilteredCount ?? 0}`,
        `afterTitleBody=${adapter?.afterTitleSnippetFilterCount ?? adapter?.normalizedResultCount ?? 0}`,
        `afterDedupe=${adapter?.afterDedupeCount ?? adapter?.normalizedResultCount ?? 0}`,
        ...
        `dropUrl=${adapter?.dropReasonBreakdown?.url_filter_removed ?? 0}`,
        `dropText=${adapter?.dropReasonBreakdown?.missing_title_or_body_removed ?? 0}`,
        `dropDate=${adapter?.dropReasonBreakdown?.date_filter_removed ?? 0}`,
        `dropDuplicate=${adapter?.dropReasonBreakdown?.duplicate_removed ?? 0}`,
