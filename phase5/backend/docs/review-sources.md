...
- Reddit: preferred live mode uses OAuth client credentials with `REDDIT_CLIENT_ID`, `REDDIT_CLIENT_SECRET`, and `REDDIT_USER_AGENT` when approved. Submission-safe mode uses `WEB_SEARCH_PROVIDER` plus `WEB_SEARCH_API_KEY` to search public Reddit pages and use titles/snippets as public discussion signals. If neither is available, the engine tries lightweight public JSON search before fallback-assisted Reddit corpus. Do not claim Reddit OAuth live mode without credentials.
...
Every run returns `sourceDiagnostics` for each selected source, including live counts, fallback counts, final counts used, and reason codes such as `missing_app_store_app_id`, `reddit_web_search_succeeded`, `reddit_auth_missing_public_fetch_limited`, `missing_web_search_provider`, or `x_bearer_token_missing_public_no_auth_unavailable`.
