/**
 * Twitter / X scraper
 * Twitter removed free API access. We use their public syndication/search
 * endpoints that don't require authentication:
 *  - https://twitter.com/search?q=gaana&f=live (public HTML)
 *  - nitter.net mirrors (multiple open instances, no auth)
 *
 * Strategy: iterate known Nitter instances, scrape tweet text with Cheerio.
 * Falls back gracefully if all Nitter instances are down.
 */
...
/** Public Nitter instances (updated 2025) */
...
async function scrapeTweetsFromNitter(
  ...
  // Only 1 attempt per instance — we have fallback instances, no need to retry
  const res = await fetchWithRetry(url, {
    headers: { "User-Agent": "Mozilla/5.0 (compatible; GaanaBot/2.0)" },
  }, 0);
  ...
  $(".timeline-item").each((_, el) => {
    ...
    // Parse date — Nitter formats as "Jan 1, 2026 · 12:00 PM UTC"
...
export async function scrapeTwitter(): Promise<ScrapeResult> {
    ...
    for (const instance of NITTER_INSTANCES) {
      ...
      try {
        ...
        for (const t of tweets) {
          if (!isWithinRange(t.date)) continue;
